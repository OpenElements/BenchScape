...
/**
 * Configuration for a benchmark. This configuration is used to configure the benchmark. It is added to a {@link BenchmarkExecution} to store the configuration of the benchmark for the given execution.
...
public record BenchmarkConfiguration(int threads, int forks, long timeout, @NonNull TimeUnit timeoutUnit,
    ...
    public BenchmarkConfiguration {
        ...
        if(forks < 1) {
            throw new IllegalArgumentException("forks must be greater than 0");
        }
        if(timeout < 1) {
